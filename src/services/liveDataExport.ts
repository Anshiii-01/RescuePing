...
export class LiveDataExportService {
  ...
  static generatePDFReport(data: ExportData): Blob {
    ...
    const content = `
RESCUE PING - DISASTER MANAGEMENT REPORT
...
` : ''}
═══════════════════════════════════════════════════════════
Report generated by Rescue Ping Disaster Management System
...
export const useLiveDataExport = () => {
  ...
  const exportToPDF = async () => {
    ...
    const filename = `rescue-ping-report-${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.txt`;
  ...
  const exportToCSV = async () => {
    ...
    const filename = `rescue-ping-data-${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    LiveDataExportService.downloadFile(blob, filename);
